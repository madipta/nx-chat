...
@Component({
  ...
  template: `
    <form
      ...
      <h2 class="text-3xl md:text-4xl text-gray-700 font-bold mb-8">Please Sign In</h2>
      <input
        type="text"
        name="username"
        ngModel
      ...
      <button 
        (click)="login()"
        class="bg-green-700 font-semibold uppercase tracking-wider text-white py-3 mb-8">Sign In</button>
      <a 
...
export class LoginComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.subscription = this.authService.onLoginResult().subscribe(res => {
      if (res) {
